  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 bg-white text-charcoal">
      <div className="border-b border-border pb-6 mb-10 text-charcoal">
        ...
        <p className="font-mono text-xs text-muted uppercase tracking-wider mt-2">
          Found {products.length} matching collaboration pieces
        </p>
      </div>

      {loading ? (
        <div className="text-center py-16 font-mono uppercase tracking-widest text-muted text-xs">Scanning Vault Archive...</div>
      ) : products.length === 0 ? (
        <div className="text-center py-20 bg-white border border-border p-8 max-w-lg mx-auto rounded-2xl shadow-subtle text-charcoal">
          ...
          <h2 className="font-serif text-2xl font-normal text-charcoal mb-2">No Matching Drops Found</h2>
...
export default function SearchPage() {
  return (
    <Suspense fallback={<div className="p-20 text-center font-mono uppercase tracking-widest text-muted">Searching Vault...</div>}>
      <SearchPageContent />
    </Suspense>
  );
}
